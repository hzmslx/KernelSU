...
int tcp_server_accept(void) {
    ...
    while (1) {
        ...
        accept_err =
                sock_create(socket->sk->sk_family, socket->type, socket->sk->sk_protocol, &accept_socket);
        ...
        accept_err = socket->ops->accept(socket, accept_socket, O_NONBLOCK, 1);
...
int tcp_server_listen(void) {
    ...
    server_err = sock_create_kern(&init_net, PF_INET, SOCK_STREAM, IPPROTO_TCP, &tcp_server->listen_socket);
    if (server_err < 0) {
    ...
    server_err = conn_socket->ops->listen(conn_socket, 5);
